...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>显卡</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/xk/go_xk_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likexk.name}"/>
            芯片厂商:<input type="text" name="xpcs" style="width: 80px" value="${sessionScope.likexk.xpcs}"/>
            显卡芯片:<input type="text" name="xkxp" style="width: 80px" value="${sessionScope.likexk.xkxp}"/>
            显存容量:<input type="text" name="xcrl" style="width: 80px" value="${sessionScope.likexk.xcrl}"/>
            显存位宽:<input type="text" name="xcwk" style="width: 80px" value="${sessionScope.likexk.xcwk}"/>
            接口:<input type="text" name="jk" style="width: 80px" value="${sessionScope.likexk.jk}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款显卡*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.allxk}" var="xk">
                <c:if test="${xk.id!=100}">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2f.zol-img.com.cn/product/183_120x90/951/ceHcydFnwKHFo.jpg" >
                    </div>
                    <div class="price-box">
                        <span class="price">￥${xk.price}</span>
                        <a href="javascript:addxk(${xk.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${xk.name} </em></span>
                            <span>芯片厂商：<em> ${xk.xpcs} </em></span>
                            <span>显卡芯片：<em> ${xk.xkxp} </em></span>
                            <span>显存容量：<em> ${xk.xcrl} </em></span>
                            <span>显存位宽：<em> ${xk.xcwk} </em></span>
                            <span>接口：<em> ${xk.jk} </em></span>
                        </div>
                    </div>
                </div>
                </c:if>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/xk/go_xk_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/xk/go_xk_diy2?currentPage=${sessionScope.currentPage-1}"
    }
