...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>内存</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/nc/go_nc_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likenc.name}"/>
            容量:<input type="text" name="rl" style="width: 80px" value="${sessionScope.likenc.rl}"/>
            容量描述:<input type="text" name="rlms" style="width: 80px" value="${sessionScope.likenc.rlms}"/>
            类型:<input type="text" name="lx" style="width: 80px" value="${sessionScope.likenc.lx}"/>
            主频:<input type="text" name="zp" style="width: 80px" value="${sessionScope.likenc.zp}"/>
            工作电压:<input type="text" name="gzdy" style="width: 80px" value="${sessionScope.likenc.gzdy}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款内存*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.allnc}" var="nc">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2f.zol-img.com.cn/product/173_120x90/791/ceZMhVCgEfffk.jpg">
                    </div>
                    <div class="price-box">
                        <span class="price">￥${nc.price}</span>
                        <a href="javascript:addnc(${nc.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${nc.name} </em></span>
                            <span>容量：<em> ${nc.rl} </em></span>
                            <span>容量描述：<em> ${nc.rlms} </em></span>
                            <span>类型：<em> ${nc.lx} </em></span>
                            <span>主频：<em> ${nc.zp} </em></span>
                            <span>工作电压：<em> ${nc.gzdy} </em></span>
                        </div>
                    </div>
                </div>
            </c:forEach>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/nc/go_nc_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/nc/go_nc_diy2?currentPage=${sessionScope.currentPage-1}"
    }
