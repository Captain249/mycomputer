...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>显示器</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/xsq/go_xsq_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likexsq.name}"/>
            屏幕尺寸:<input type="text" name="pmcc" style="width: 80px" value="${sessionScope.likexsq.pmcc}"/>
            产品类型:<input type="text" name="cplx" style="width: 80px" value="${sessionScope.likexsq.cplx}"/>
            视频接口:<input type="text" name="spjk" style="width: 80px" value="${sessionScope.likexsq.spjk}"/>
            面板类型:<input type="text" name="mblx" style="width: 80px" value="${sessionScope.likexsq.mblx}"/>
            最佳分辨率:<input type="text" name="zjfbl" style="width: 80px" value="${sessionScope.likexsq.zjfbl}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款显示器*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.allxsq}" var="xsq">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2c.zol-img.com.cn/product/181_120x90/522/ceq78IpidgA.jpg">
                    </div>
                    <div class="price-box">
                        <span class="price">￥${xsq.price}</span>
                        <a href="javascript:addxsq(${xsq.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${xsq.name} </em></span>
                            <span>屏幕尺寸：<em> ${xsq.pmcc} </em></span>
                            <span>产品类型：<em> ${xsq.cplx} </em></span>
                            <span>视频接口：<em> ${xsq.spjk} </em></span>
                            <span>面板类型：<em> ${xsq.mblx} </em></span>
                            <span>最佳分辨率：<em> ${xsq.zjfbl} </em></span>
                        </div>
                    </div>
                </div>
            </c:forEach>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/xsq/go_xsq_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/xsq/go_xsq_diy2?currentPage=${sessionScope.currentPage-1}"
    }
