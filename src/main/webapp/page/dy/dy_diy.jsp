...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>电源</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/dy/go_dy_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likedy.name}"/>
            额定功率:<input type="text" name="edgl" style="width: 80px" value="${sessionScope.likedy.edgl}"/>
            PFC类型:<input type="text" name="pfclx" style="width: 80px" value="${sessionScope.likedy.pfclx}"/>
            电源类型:<input type="text" name="dylx" style="width: 80px" value="${sessionScope.likedy.dylx}"/>
            风扇描述:<input type="text" name="fsms" style="width: 80px" value="${sessionScope.likedy.fsms}"/>
            其他特点:<input type="text" name="qttd" style="width: 80px" value="${sessionScope.likedy.qttd}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款电源*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.alldy}" var="dy">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2b.zol-img.com.cn/product/190_120x90/313/ceSTzVI8Uc1jU.jpg">
                    </div>
                    <div class="price-box">
                        <span class="price">￥${dy.price}</span>
                        <a href="javascript:adddy(${dy.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${dy.name} </em></span>
                            <span>额定功率：<em> ${dy.edgl} </em></span>
                            <span>PFC类型：<em> ${dy.pfclx} </em></span>
                            <span>电源类型：<em> ${dy.dylx} </em></span>
                            <span>风扇描述：<em> ${dy.fsms} </em></span>
                            <span>其他特点：<em> ${dy.qttd} </em></span>
                        </div>
                    </div>
                </div>
            </c:forEach>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/dy/go_dy_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/dy/go_dy_diy2?currentPage=${sessionScope.currentPage-1}"
    }
