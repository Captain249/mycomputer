...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>主板</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/zb/go_zb_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likezb.name}"/>
            主芯片组:<input type="text" name="zxpz" style="width: 80px" value="${sessionScope.likezb.zxpz}"/>
            CPU插槽:<input type="text" name="cc" style="width: 80px" value="${sessionScope.likezb.cc}"/>
            主板板型:<input type="text" name="bx" style="width: 80px" value="${sessionScope.likezb.bx}"/>
            集成芯片:<input type="text" name="jcxp" style="width: 80px" value="${sessionScope.likezb.jcxp}"/>
            内存类型:<input type="text" name="nclx" style="width: 80px" value="${sessionScope.likezb.nclx}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款主板*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.allzb}" var="zb">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2a.zol-img.com.cn/product/190_120x90/490/ceDcYi3xd7Ca2.jpg">
                    </div>
                    <div class="price-box">
                        <span class="price">￥${zb.price}</span>
                        <a href="javascript:addzb(${zb.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${zb.name} </em></span>
                            <span>主芯片组：<em> ${zb.zxpz} </em></span>
                            <span>CPU插槽：<em> ${zb.cc} </em></span>
                            <span>主板板型：<em> ${zb.bx} </em></span>
                            <span>集成芯片：<em> ${zb.jcxp} </em></span>
                            <span>内存类型：<em> ${zb.nclx} </em></span>
                        </div>
                    </div>
                </div>
            </c:forEach>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/zb/go_zb_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/zb/go_zb_diy2?currentPage=${sessionScope.currentPage-1}"
    }
