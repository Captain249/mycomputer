...
<div class="conditions">
    <div class="list-title">
        <div id="J_CityArea" class="list-title-inner clearfix" rel-sid="28">
            <h3>请选择<font>CPU</font></h3>
        </div>
    </div><!--list-title end-->
    <div id="J_ParamFilter" class="param-filter" style="height: 39px;">
        <form action="<%=path %>/cpu/go_cpu_diy3" method="post" target="ifmname">
            型号:<input type="text" name="name" style="width: 80px" value="${sessionScope.likecpu.name}"/>
            插槽类型:<input type="text" name="cc" style="width: 80px" value="${sessionScope.likecpu.cc}"/>
            CPU主频:<input type="text" name="zp" style="width: 80px" value="${sessionScope.likecpu.zp}"/>
            动态加速频率:<input type="text" name="dtjspl" style="width: 80px" value="${sessionScope.likecpu.dtjspl}"/>
            三级缓存:<input type="text" name="sjhc" style="width: 80px" value="${sessionScope.likecpu.sjhc}"/>
            核心数量:<input type="text" name="hxsl" style="width: 80px" value="${sessionScope.likecpu.hxsl}"/>
    ...
    <div class="sort-box clearfix">
        ...
        <span class="total totalNum">共 <b>${sessionScope.total}</b> 款CPU*</span>
    </div>
    ...
    <div class="list-box" style="display: block;">
        <div class="page-box">
            <%--迭代对象--%>
            <c:forEach items="${sessionScope.allcpu}" var="cpu">
                <div class="pitem clearfix s_1177026 p_1177026" reldate="1177026" relmanu="125">
                    <div class="pic">
                            <img src="https://2f.zol-img.com.cn/product/190_120x90/91/ce0ZgsXUEw8cA.jpg" alt="Intel 酷睿i7 8700K">
                    </div>
                    <div class="price-box">
                        <span class="price">￥${cpu.price}</span>
                        <a href="javascript:addcpu(${cpu.id});" target="_self" class="add"><i></i>加入配置单</a>
                    </div>
                    <div class="pro-intro">
                        <div class="paramet">
                            <span>型号：<em> ${cpu.name} </em></span>
                            <span>插槽类型：<em> ${cpu.cc} </em></span>
                            <span>CPU主频：<em> ${cpu.zp} </em></span>
                            <span>动态加速频率：<em> ${cpu.dtjspl} </em></span>
                            <span>三级缓存：<em> ${cpu.sjhc} </em></span>
                            <span>核心数量：<em> ${cpu.hxsl} </em></span>
                        </div>
                    </div>
                </div>
            </c:forEach>
...
<script>
    ...
    function nextPage() {
        window.location.href="<%=path %>/cpu/go_cpu_diy2?currentPage=${sessionScope.currentPage+1}"
    }
    function lastPage() {
        window.location.href="<%=path %>/cpu/go_cpu_diy2?currentPage=${sessionScope.currentPage-1}"
    }
